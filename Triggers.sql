...
-- insert into dim_productos (id_juego, id_consola, id_tipo, precio) values (1,1,1,3300);
...
begin
	...
	if (exists( select nombre, id_clasificacion, id_categoria from dim_juegos where 
		nombre = new.nombre and
        id_categoria = new.id_categoria and
        id_clasificacion = new.id_clasificacion )) 
        then
			signal sqlstate "45000"
			set message_text = error_ord;
    end if;
...
-- insert into dim_juegos (nombre, id_categoria, id_clasificacion) values ("Mortal Kombat 11", 7, 5);